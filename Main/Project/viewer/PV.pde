...
void drawPlane2(float theta, float tilt, float eps){
  pt A = P(-100, 0, 100);
  pt B = P(100, 0, 100);
  pt C = P(-100, 0, -100);
  pt D = P(100, 0, -100);  
...
float[] solveAngles(float x1, float z1, float x2, float z2, float x3, float z3, float x4, float z4, float f){
  ...
  if(E == 0 || F == 0 ||  (A*(x1-x3) - B*(x2-x4) - C*(x1-x2) + D*(x3-x4)) == 0){
    ...
    angles[2] = -1.0 * atan((A*(z1-z3) - B*(z2-z4) - C*(z1-z2) + D*(z3-z4))/
